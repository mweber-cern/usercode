...
bool LumiRanges::check( const ID LS ) const {
   ...
   for( vector< range >::const_iterator r = ranges.begin(); r != ranges.end(); ++r ) {
      if( r->first <= LS && r->second >= LS ) return true;
   }
...
RunLumiRanges::RunLumiRanges( const string &filename ) {
   ...
   if( !ifs.good() ) throw bad_config( "Could not read run/lumi config: "+filename );
   ...
   try {
      ...
      while( ! ifs.eof() ) {
         ...
         ifs >> c;
         if( c != '"' ) throw bad_config( filename, c );
         ...
         while( true ){
            ...
            ifs >> c;
            if( c == ']' ) break; //this run is done
            else if( c != ',' ) throw bad_config( filename, c );
         }
         ...
         //read the next char, which must be either a , or a }
         ifs >> c;
         if( c == '}' ) break; //this config is done
         else if( c != ',' ) throw bad_config( filename, c );
      }
      ...
      if( ifs.eof() ) throw bad_config( filename, "Unexpected EOF" );
      //reading failed, but the stream itself is good
      if( ifs.fail() && !ifs.bad() ) throw bad_config( filename, "Read failed (probably an unexpected character)" );
      //the stream is bad
      if( ifs.bad() ) throw bad_config( "Error while reading: "+filename );
   }
}
...
bool RunLumiRanges::check( const ID run, const ID LS ) {
   if( empty ) { //empty config means accept all
      last_state = true;
   } else if( last_run != run || first ) { //we are either in a new run or in the first round
   ...
   } else if( last_LS != LS ) { //the run hasn't changed, but the LS
      //update the cache
      last_LS = LS;
